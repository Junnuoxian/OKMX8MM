...
A53 会检查相邻批次的 `sequence` 是否连续；如果发现丢批或乱序，本轮处理会停止，避免错误数据继续写入后续输出。
...
- MQTT 待发送文件每行包含 `sequence`、`qos`、`topic` 和 `payload`。
- 发布脚本使用 `MQTT_HOST`、`MQTT_PORT`、`MQTT_QOS`、`MQTT_USER`、`MQTT_PASSWORD`。
...
- 人为写入不完整尾部后，`--recover-storage runtime-data/a53-storage.jsonl` 能恢复。
