...
static int apply_config_value(a53_cli_options_t *options, const char *key, const char *value)
{
    ...
    if (strcmp(key, "heartbeat") == 0) {
        if (copy_option_text(options->heartbeat_path_value,
                sizeof(options->heartbeat_path_value),
                value) != 0) {
            return -1;
        }
        options->heartbeat_path = options->heartbeat_path_value;
        return 0;
    }
...
int a53_cli_parse(int argc, const char **argv, a53_cli_options_t *options)
{
    ...
    options->heartbeat_path = "runtime-data/a53-heartbeat.jsonl";
    ...
    options->heartbeat_path_value[0] = '\0';
    ...
    for (index = 1; index < argc; index++) {
        ...
        } else if (strcmp(argv[index], "--heartbeat") == 0) {
            if (index + 1 >= argc) {
                return -1;
            }
            options->heartbeat_path = argv[++index];
