...
static int collect_modbus_rtu_data_demo3_gateway(
    modbus_rtu_context_t *modbus_rtu_context,
    modbus_rtu_t *modbus_rtu,
    modbus_rtu_device_t *device,
    demo3_sample_t *sample);

static int save_demo3_sample(
    modbus_rtu_context_t *modbus_rtu_context,
    demo3_sample_t *sample);
...
static int collect_modbus_rtu_data(modbus_rtu_context_t *modbus_rtu_context, modbus_rtu_t *modbus_rtu) {
    ...
    for (int i = 0; i < device_count; i++) {
        ...
        } else if (device->device_type == DEVICE_TYPE_DEMO3_GATEWAY) {
            device_context->metrics_list = (demo3_sample_t *)malloc(sizeof(demo3_sample_t) * device_context->metrics_list_size);
            device_context->metrics = (demo3_sample_t *)malloc(sizeof(demo3_sample_t));
        } else {
    ...
    while (shutdown_flag == 0) {
        ...
        for (int device_index = 0; device_index < device_count; device_index++) {
            ...
            } else if (device_type == DEVICE_TYPE_DEMO3_GATEWAY) {
                demo3_sample_t *sample_list = (demo3_sample_t *)device_context->metrics_list;
                demo3_sample_t *sample = sample_list + device_context->schedule_index;
                collect_ret = collect_modbus_rtu_data_demo3_gateway(modbus_rtu_context, modbus_rtu, device, sample);
            }
        ...
        || modbus_rtu_context->next_publish_time - time_now_millis > modbus_rtu->publish_interval * 10) {
            ...
            for (int i = 0; i < device_count; i++) {
                ...
                if (device->device_type == DEVICE_TYPE_OIL_SENSOR) {
                    ...
                    demo3_sample_t sample;
                    demo3_sample_t *sample_list = (demo3_sample_t *)device_context->metrics_list;
                    int latest_index = device_context->schedule_index == 0
                        ? device_context->metrics_list_size - 1
                        : device_context->schedule_index - 1;

                    memset(&sample, 0, sizeof(sample));
                    sample.timestamp_ms = (uint32_t)time_now_millis;
                    if (device_context->schedule_count > 0) {
                        sample = sample_list[latest_index];
                    } else {
                        sample.flags = DEMO3_SAMPLE_RS485_ERROR;
                    }
                    *((demo3_sample_t *)device_context->metrics) = sample;

                    if (save_demo3_sample(modbus_rtu_context, &sample) != 0) {
                        overall_error_code = 1;
                    }
                }
...
static int collect_modbus_rtu_data_demo3_gateway(
    ...
    modbus_rtu_device_t *device,
    demo3_sample_t *sample) {
    uint8_t data[DEMO3_MODBUS_REGISTER_COUNT * 2u];
    uint16_t registers[DEMO3_MODBUS_REGISTER_COUNT];
    int read_ret;
    size_t i;

    memset(data, 0, sizeof(data));
    read_ret = modbus_rtu_read_registers(
        modbus_rtu_context,
        modbus_rtu,
        device,
        0x00,
    ...
    for (i = 0u; i < DEMO3_MODBUS_REGISTER_COUNT; ++i) {
        registers[i] = (uint16_t)(((uint16_t)data[i * 2u] << 8u) | data[i * 2u + 1u]);
    }
    if (demo3_modbus_decode_registers(registers, DEMO3_MODBUS_REGISTER_COUNT, sample) != 0) {
        return -1;
    }
    sample->timestamp_ms = (uint32_t)get_time_millis();
...
static int save_demo3_sample(
    modbus_rtu_context_t *modbus_rtu_context,
    demo3_sample_t *sample) {
    modbus_rtu_t *modbus_rtu = modbus_rtu_context->modbus_rtu_config;
    int has_error = 0;
    int i;

    if (sample == NULL || modbus_rtu_context->file_storage_contexts == NULL) {
        return -1;
    }
    for (i = 0; i < modbus_rtu->file_storage_config_count; ++i) {
        file_storage_context_t *storage = modbus_rtu_context->file_storage_contexts + i;
        if (file_storage_write(storage, (char *)sample, (int)sizeof(*sample)) != 0) {
            has_error = 1;
        }
    }
